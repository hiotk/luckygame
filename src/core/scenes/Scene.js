...
import Aactive from '../../assets/A_active.svg';
import B from '../../assets/B.svg';
import Bactive from '../../assets/B_active.svg';
import C from '../../assets/C.svg';
import Cactive from '../../assets/C_active.svg';
import D from '../../assets/D.svg';
import Dactive from '../../assets/D_active.svg';
import G from '../../assets/G.svg';
import Gactive from '../../assets/G_active.svg';
import {
    MIN_SCALE,
    MAX_SCALE,
    CANVAS_EVENTS,
...
export default class Scene {
    ...
    Render(frameNum) {
        ...
        if (!this.context || this.frameNum <= 0) {
            return;
        }
        // 清除画布
    ...
    loaderStatic() {
        const svgs = {
            A, Aactive, B, Bactive, C, Cactive, D, Dactive, G, Gactive
        };
        const svgImgs = {};
    ...
    */
        ...
        if (this[`$${canvasEvent.eventType}`] && this[`$${canvasEvent.eventType}`].length) {
            this[`$${canvasEvent.eventType}`].forEach((callback) => {
                Promise.resolve().then(() => {
                    ...
                    // 收起 或 展开 需要执行动画；需要ANIMATION_FRAME_NUM帧完成动画
                    this.refresh(canvasEvent.eventType === 'clickCollapse' || canvasEvent.eventType === 'clickExpand' ? ANIMATION_FRAME_NUM : 1);
                }); // 防止阻塞进程
            });
        }
    }
    ...
    destryImg() {
        if (this.svgImgs) {
            Object.keys(this.svgImgs).forEach(key => {
                this.svgImgs[key].then((img) => {
                    if (img && img.remove) {
                        img.remove();
                    }
                });
    ...
    changeScale(increment) {
        ...
        this._calcScale(this.canvasWidth / 2, this.canvasHeight / 2, increment);
        this.refresh(1);
    }
  ...
  */
    ...
    resize() {
        ...
        this.canvas.width = this.canvasWidth;
        this.canvas.height = this.canvasHeight;
        this.translateX += (this.canvasWidth - lastWidth) / 2;
        this.translateY += (this.canvasHeight - lastHeight) / 2;
        this._calcCanvasRange();
        this.refresh(1);
    }
}
